...
const filters = new Array( 'Owner', 'Group', 'Manufacturer', 'Brand','Finish','Colour','Weight','Quality', 'Size','Material','type','width','height' );

function filter_onChange( element, id, selected ) {
	var form = element.form;
	var h = [];
  h[h.length] = {name: 'form', value: form.id};
  h[h.length] = {name: 'id', value: id};
  h[h.length] = {name: 'selected', value: element.name};
	for ( var index = 0, len = filters.length; index < len; ++index ) {
		var filter = form.elements[filters[index]+id];
		if ( filter ) {
			h[h.length] = {name: filters[index], value: $j(filter).val() };
			if ( filter.type == 'select-one' ) {
			filter.disabled = true;
			} // end if
			filter = form.elements[filters[index]+'_exclude'+id];
			if ( filter ) {
				h[h.length] = {name:filter.name, value: $j(filter).val()};
			} // end if
		...
		if ( filter ) {
			...
			h[h.length] = {name:filter.name, value: v };
			if ( filter.type == 'select-one' ) {
				filter.disabled = true;
			} // end if
			...
			if ( filter ) {
				var v = $j(filter).val();
				if ( v ) h[h.length] = {name:filter.name, value: v };
			} // end if
		} // end if filter exists
	} // end for 
  //
  $j.ajax({
      type: "POST",
      url: '/administrator/stock/_stock.json',
      data: h,
      dataType: 'json',
...
function cbStockFillResults( results ) {
  console.log('cbStockFillResults', results);
  if (!results.form) {
    alert('No form in results.');
    return;
  }
