...
sub send_sales_order {
  ...
  my $cust_id = scalar $dbh->selectrow_array(q{ SELECT lngCustomerid FROM tbl_orders WHERE lngorderid = ?  }, undef, $order_id);
  ...
  my @additional_attachments = misc::get_attachments(openprint::misc::get_files($openprint::config{SkinPath}.'/email/order/'));
  ...
  if ( $pending ) {
    ...
    misc::send_email_with_attachment($r, $log, \%mail, @body, @project_summaries, @additional_attachments ) if $sales_email;
    delete $mail{BODY};
    ...
    misc::send_email_with_attachment($r, $log, \%mail, @body, @project_summaries, @additional_attachments ) if $notificationemail;
    delete $mail{BODY};
