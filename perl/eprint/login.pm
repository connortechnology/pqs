...
sub verify_login {
  ...
  $_ = "SELECT ysnAccountActivation FROM tbl_Customer WHERE lngCustomerID = '$cust_id'";
  ...
  $openprint::User = new openprint::User($user_id);
  $openprint::Company = new openprint::Company($cust_id);
  ...
  $cookie = Apache2::Cookie->new($r,
    -name   => 'username',
    -value  => $openprint::User->name(),
    -path   => '/',
    -domain => $r->dir_config('cookiedomain'));
  $cookie->bake($r);
  $variable->{user}{name} = $openprint::User->name();
