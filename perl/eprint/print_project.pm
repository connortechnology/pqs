...
# Returns a hashref of categories that contain an array of project type hashes for easy use with SSI.
sub project_types {
  my ($log, $dbh) = @_;
...
sub service_types_by_category {
  ...
  for my $cat (@$categories) {
    ...
    @services = grep { exists $allowed->{ $_->{id} } } @services if $allowed && !$is_staff;
...
sub edit_process {
  ...
  $project->set({
      (defined($r->param('txtComments')) ? (comments=> $r->param('txtComments')) : ()),
      (defined($r->param('invoice_comments')) ? (invoice_comments => $r->param('txtInvoiceComments') ) : ()),
      (defined($r->param('txtProjectReference')) ? (reference => $r->param('txtProjectReference')) : () ),
      (defined($r->param('rdbPressType')) ? (press_type => $r->param('rdbPressType')) : () ),
      (defined($r->param('press_type_id')) ? (press_type_id => $r->param('press_type_id')) : () ),
      (defined($r->param('rdbProjectType')) ? (type_id => $r->param('rdbProjectType')) : () ),
      (defined($r->param('rfq_only')) ? ( rfq_only => $r->param('rfq_only') ) : ()),
    });
...
sub display_reuse_project {
  ...
  my $o = $dbh->selectrow_array(q{ SELECT count(*) FROM tbl_order_contents WHERE lngprojectindex = ?  }, undef, $variable->{ProjectIndex});

  my $q = $dbh->selectrow_array(q{ SELECT count(*) FROM tbl_quote_details WHERE lngprojectindex = ?  }, undef, $variable->{ProjectIndex});
...
sub copy_project_services {
  ...
  my $project = openprint::Project->find_one(id=>$src);
  my $new_project = openprint::Project->find_one(id=>$dest);
  die "Missing a project $src, $dest" if !($project and $new_project);
  ...
  while (my $contents = $sth->fetchrow_hashref) {
    ...
    while ( $specs->fetch ) {
      ...
      # Cripple the shipping information so it will get the address for the new customer.
      ...
      if ($name eq 'txtQuantity1' and ($project->quantity1() != $new_project->quantity1())) {
        $value = $new_project->quantity1();
      } elsif ($name eq 'txtQuantity2' and ($project->quantity2() != $new_project->quantity2())) {
        $value = $new_project->quantity2();
      } elsif ($name eq 'txtQuantity3' and ($project->quantity3() != $new_project->quantity3())) {
        $value = $new_project->quantity3();
      }
    ...
    if ( $contents->{strservicetype} eq 'Shipping' ) {
      ...
      while (my $ship = $old_ship->fetchrow_hashref) {
        ...
        my $new_id = $dbh->last_insert_id('', qw(public tbl_addresses lngindex));
...
sub recalculate_service {
  my ($r, $log, $dbh, $cookie, $var, $pid) = @_;

  my $sid = $openprint::param{sid};
  my $project = new openprint::Project($pid);
  my $service = $project->Service($sid);
  my $service_type = $service->ServiceType();
  $service_type = eprint::service::load_service_type($service_type);
  my %template = eprint::project::template_service_types($log, $dbh, $pid);
  my $type = $$service_type{name};
  $service_type->{is_needed} = eprint::Build::needed($log, $dbh, $pid, $service_type, $template{$type});

  my $status = eval {
    set_need($dbh, $sid, $service_type->{is_needed}); # Set (new) need level.

    my $form  = eprint::service::get_specs($dbh, $pid, $sid, $service_type, 1); # User defined
    my $specs = eprint::service::get_specs($dbh, $pid, $sid, $service_type);    # All specs

    # Price the service.
    my $state = eprint::service::price($log, $dbh, \%openprint::variable, $pid, $sid, $service_type, $specs, 1);

    # Save the specifications and pricing.
    eprint::service::save($log, $dbh, $pid, $sid, $service_type, $form, $specs) if $state eq 'calculated';

    return $state;
  };
  #die; # if $status ne 'calculated';
  set_status($log, $dbh, $pid, $status, $sid);

  #eprint::Build::process($log, $dbh, \%openprint::variable, $pid, $sid, $service_type);
...
sub create_project {
  ...
  $dbh->do(q{UPDATE tbl_projects SET create_to_order = true WHERE lngprojectindex = ?}, undef, $pid) if $r->param('create_to_order');
  ...
  if ( $r->param('mail_type') ) {
    ...
    $dbh->do(q{ UPDATE tbl_projects SET mail_type = ?, mail_art  = ?  WHERE lngprojectindex = ?  }, undef, $type, $art, $pid);
  }
  ...
  if ( $r->param('mail_type') ) {
    eprint::mailing::reserve_mail($r, $dbh, $variable, $pid, $r->param('txtQuantity1'));
  ...
  return (has_pdf_template(undef, $dbh, $pid) ? TEMPLATE_PAGE : BUILD_PAGE) . "?$p";
}
...
sub copy {
  ...
  return $changed ? BUILD_PAGE . "?pid=$new;level=0" : VIEW_PAGE  . "?pid=$new";
}
...
sub project_notification {
  ...
  my $info;
  if ( $r->param('pid' ) ) {
    $dbh->do(q{ DELETE FROM tbl_order_contents WHERE lngprojectindex = ?  }, undef, $r->param('pid'));
    $dbh->do(q{ DELETE FROM tbl_quote_details WHERE lngprojectindex = ?  }, undef, $r->param('pid'));

    $info = $dbh->selectall_arrayref(q{SELECT name, value FROM hybird_specs WHERE pid = ?},{Slice=>{}}, $r->param('pid'));
    ...
    $info = $dbh->selectall_arrayref(q{ SELECT name, value FROM hybird_specs WHERE req = ?  },{Slice=>{}}, $r->param('id'));
  ...
  $data->{item} = $dbh->selectrow_array(q{ SELECT name FROM product.item WHERE id = ?}, undef, $data->{item});

  my $user = new openprint::User($data->{ddmUser});
  $data->{ddmUser} = $user->name();
  ...
  my $email = $dbh->selectrow_array(q{ select notificationemail from tbl_customer where strcompanyname = 'SAFEWAY'; });
  if ($email) {
    my %mail = (
