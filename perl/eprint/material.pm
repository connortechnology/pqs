...
sub get_price {
    ...
    if ( $material =~ /^\d+$/ ) {
        ...
        $mi = scalar $dbh->selectrow_array(q{
                    SELECT lngindex FROM tbl_materials WHERE lngindex = ?
        }, undef, $material);
    }

    # Fetch the numberic ID if they're passed us a string.
    $mi = scalar $dbh->selectrow_array(q{
        SELECT lngindex FROM tbl_materials WHERE strid = ?
    }, undef, $material) unless $mi;
