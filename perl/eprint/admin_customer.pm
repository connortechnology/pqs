...
sub markup_cats {
  my $dbh = session::dbh;
  my $list = $dbh->selectcol_arrayref(q{ SELECT id FROM categories ORDER by name }  );
  return $list;
}
...
# add them back in 
  ...
  if ( $index ) {
    ...
    my $cust_service_types = $dbh->selectcol_arrayref(q{
      SELECT service_type FROM customer_service_type WHERE customer = ?  }, undef, $index);

    my $service_types = $dbh->selectall_arrayref(q{
      SELECT lngindex, strname FROM tbl_service_types ORDER BY strname });
