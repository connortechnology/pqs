...
# Used to get the quantity of a given service type.
sub get_quantity {
  ...
  my $sth = $dbh->prepare(
    ...
    WHERE s.lngprojectindex = ?  AND s.lngserviceindex = ?
...
sub setup_RFQ {
  ...
  my @date = gmtime;
  $variable->{rfq}{date_created} = ($date[5] + 1900) . '-' . $date[4] . '-' . $date[3];
...
# Dispatch Function used to return information about a given service type.
sub summary {
...
sub comment {
  ...
  if ( $ctype ) {
    if ($r->param('Save')) {
      sql::update($log, $dbh, 'tbl_projects', "lngprojectindex = $pid", $ctype, $comment) if $comment;
      $dbh->do(qq{ update tbl_projects SET $ctype = NULL WHERE lngprojectindex = ?}, undef, $pid) unless $comment;
    }
    ($comment) = $dbh->selectrow_array( qq{SELECT $ctype FROM tbl_projects WHERE lngprojectindex =?}, undef, $pid);
  } else {
    sql::update(
      $log, $dbh, 'tbl_project_contents', ['lngprojectindex=? AND lngserviceindex=?', $pid, $sid],
      'strcomments', $comment) if $r->param('Save');

    sql::update(
      $log, $dbh, 'tbl_project_contents', ['lngprojectindex=? AND lngserviceindex=?', $pid, $sid],
      'trackingnumber', $trackingnumber) if $trackingnumber;
  ...
  $variable->{order_id} = scalar $dbh->selectrow_array( q{SELECT lngorderid FROM tbl_order_contents WHERE lngprojectindex=?}, undef, $pid);
...
sub service_summary {
  ...
  my $qtyIndex = scalar $dbh->selectrow_array(q{
    SELECT intquantityIndex FROM tbl_order_contents WHERE lngprojectindex = ?  }, undef, $pid);
  ...
  if ($order_id) {
    $expectedDate = scalar $dbh->selectrow_array(
      q{ SELECT daterequired FROM tbl_order_contents WHERE lngorderid = ?
      }, undef, $order_id);

    $status = scalar $dbh->selectrow_array(
      q{ SELECT strstatus FROM tbl_orders WHERE lngorderid = ?
      }, undef, $order_id);
    $type = 'order';
  }
...
# for die cutting, it can get any information which is in the
...
sub printing {
  ...
  my $press_type = scalar $dbh->selectrow_array(
    q{ SELECT strid FROM tbl_equipment_type
    WHERE lngindex = 
    ( SELECT lngpresstype
    FROM tbl_projects
    WHERE lngprojectindex = ?
  ...
  $results{txtServiceDescription} = $results{ProjectType} if (!(eprint::project::is_multipage($log, $dbh, $pid)));
  ...
  for my $s ( 's0','s1' ) {
    ...
    if $specs->{$s."_varnish_spot_gloss"}{value} || $specs->{$s."_varnish_spot_matte"}{value};
  ...
  if ($specs->{rdbSpecificStock}{value} eq 'Y') {
    $results{stock_name} = $specs->{txtSpecificStockBrand}{value};
    $results{stock_finish} = $specs->{txtSpecificStockFinish}{value};
    $results{stock_colour} = $specs->{txtSpecificStockColour}{value};
    $results{stock_weight} = $specs->{txtSpecificStockWeight}{value};
  } else {
    @results{'stock_name','stock_finish','stock_weight','stock_colour','stock_supplied','stock_custom'} = (
    $$specs{stock_name}{value},
    $$specs{stock_finish}{value},
    $$specs{stock_weight}{value},
    $$specs{stock_colour}{value},
    $$specs{stock_supplied}{value},
    $$specs{stock_custom}{value}
  );
  }

  my $alt_name = join(' ', '', @results{qw(stock_name stock_finish stock_colour stock_weight)}, '');

  my $cover = $dbh->selectrow_hashref(q{SELECT * FROM cover_specs WHERE pid = ?}, undef, $pid);
  $results{cover} = $cover;
  ...
  my $paper;
  if (($results{'stock_supplied'} ne 'Yes') && ($project_type ne 'ScreenItem')) {
    my $coated;
    if ($results{'pressType'} ne 'web') {
      ...
      $paper .= " - " . $results{"txtSpecificStockBrand"} if $results{"txtSpecificStockBrand"};
      $paper .= ", "
...
sub display {
  ...
  if ( $r->param('InternalID') ) {
    $dbh->do(q{ UPDATE tbl_projects set internalid = ? WHERE lngprojectindex = ?  },undef, $r->param('InternalID'), $pid);
  }
...
sub setup_docket {
  ...
  $variable->{ProjectIndex} = $pid;
  $variable->{email} = ($catID == -2) ? 1 : 0;
  $variable->{'multipage'} = eprint::project::is_multipage($log, $dbh, $pid);
  $variable->{summary} = $r->param('Summary') ? 1 : 0;
  ...
  $variable->{Category}   = $catID if !$variable->{Category};
