...
sub view_services {
  ...
  #### CUSTOM CODE SECTION ***********
  my ($q1, $q2, $q3, $digifed) = $project->get('quantity1','quantity2','quantity3','digifed');

  if ( $q2 ) {
    $project->save({quantity2=>undef});
    $qtys->[0] = $q2;
    eprint::print_project::add_qty($r, $log, $dbh, $cookie, $variable, $pid, $qtys);
  }
  if ( $q3 ) {
    $project->save({quantity3=>undef});
    $qtys->[0] = $q3;
    eprint::print_project::add_qty($r, $log, $dbh, $cookie, $variable, $pid, $qtys);
  }
  ...
  add_custom_sort($dbh, $pid, $variable) if $r->param('add_custom_sort');
  ...
  if ( $r->param('supplier') and ($r->param('supplier') ne $project->supplier())) {
    $project->save({supplier=>$r->param('supplier')});
  }
  my $sql = q{SELECT lngcustomerid, strCompanyName FROM tbl_customer WHERE ysnsupplier = 'Y' order by 2};
  $$variable{SUPPLIERS} = ssi::fill_drop_down($log, $dbh, $sql, $project->supplier());

  $variable->{quote_id} =  $dbh->selectrow_array(q{SELECT max(lngquoteid) FROM tbl_quote_details WHERE lngprojectindex=?}, undef, $pid);
  $variable->{order_id} =  $dbh->selectrow_array(q{SELECT max(lngorderid) FROM tbl_order_contents WHERE lngprojectindex=?}, undef, $pid);
  $variable->{product} =  $project->prod();
...
sub display_project {
  ...
  my $project = new openprint::Project($pid);
  my $rfq_only = $project->rfq_only();

  my $have_disc = $dbh->selectrow_array(q{
    SELECT COUNT(lngserviceindex) FROM tbl_project_contents WHERE lngprojectindex = ? AND strservicetype = 'Discount'
    }, undef, $pid);
  ...
  my %handler_for = (
    ...
    Custom => sub {
      my ($s) = @_;
      my ($name) = get_specifications($log, $dbh, $pid, $s->{id}, 'ServiceName');
      $name =~ s/^\s+(.*?)\s+$/$1/; # Expensive trim. How would it get white space in it? Should be trimmed when added
  ...
  while (my $service = $services->fetchrow_hashref) {
    ...
    foreach my $i ( 1 .. 3 ) {
      ...
      if ($service->{removed} or $qty[$i] <= 0 or $prices{"txtPrice$i"} =~ m#n/a#i) {
        $service->{"price$i"} = '';
        next QUANTITY;
      }
      ...
      if ( not grep {$service->{status} eq $_}
        ('calculated', 'In Production', 'Complete', 'Pending Deposit', 'Pending Date Approval', 'Waiting For Files') )
      {
        $service->{"price$i"} = '-';
        next QUANTITY;
      }
      ...
      $service->{"price$i"} += $stock_price->{$service->{id}}[$i-1]
        if ! $flags{stock_seperate} && $service->{ref} eq 'Printing' && exists $stock_price->{ $service->{id} };
    ...
    # Don't display any edit controls for fixed price project services that aren't in the 'allowed' additional services.
    if (!$variable->{is_staff} && $has_locked_services && !exists $allowed->{ $service->{type} } ) {
      delete $service->{url};
      ...
      if ($openprint::User->email() =~ /iconnor/) {
        push @controls, {
          name => 'Calc',
          url => "/main/proj/dispatch.html?action=calculate;pid=$pid;sid=$service->{id}"
        };
      }
  ...
  if ($is_multipage) {
    ...
    $cat->{url}   = $project_type eq 'ScreenItem' ? 'item' : 'book' if $flags{can_edit};
  } 

  my @array_of_categories = map { { name => $_, %{$category{$_}} } } keys %category;
  #print STDERR "HAVE SERVICES :", Dumper(\@array_of_categories);
  # Sort services by name.
  foreach my $cat_hash (@array_of_categories) {
    @{$$cat_hash{services}} = sort { $a->{name} cmp $b->{name} } @{$cat_hash->{services}} if @{$cat_hash->{services}};
  }
  ...
  $variable->{categories} = [ sort { $a->{cat_sort} <=> $b->{cat_sort} } @array_of_categories ];
  ...
  for my $i (1..3) {
    $variable->{unit_price}[$i] = $variable->{total_price}[$i] / $qty[$i] if $qty[$i];
  }
