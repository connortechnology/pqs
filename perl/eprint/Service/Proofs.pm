...
sub calc {
  ...
  my ($scanning_proof) = sql::sql_statement($log, $dbh, qq{
    SELECT strvalue FROM tbl_service_specifications WHERE lngprojectindex = $pid AND strname='rdbRandomProof' ORDER BY strvalue DESC
    });

  # Check to see if we have already calculated this service.
  my $prev_status = scalar $dbh->selectrow_array(q{ SELECT strstatus FROM tbl_project_contents WHERE lngserviceindex = ?  }, undef, $sid);
  ...
  ($specs->{'txtQuantity1'}, $specs->{'txtQuantity2'}, $specs->{'txtQuantity3'}) = get_specifications($log, $dbh, undef, $sid, 'txtQuantity1', 'txtQuantity2', 'txtQuantity3');
  ...
  my ($plate_changes) = $dbh->selectrow_array(q{
    SELECT strvalue FROM tbl_service_specifications WHERE strname='add_plates' and lngprojectindex = ?  ORDER BY lngserviceindex
    }, undef, $pid );

  my ($total_plates) = $dbh->selectrow_array(q{
    SELECT strvalue FROM tbl_service_specifications WHERE strname='txtPlateQuantity' and lngprojectindex = ?  ORDER BY lngserviceindex
    }, undef, $pid);
