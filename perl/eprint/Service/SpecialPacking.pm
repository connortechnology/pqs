package eprint::Service::SpecialPacking;
use strict;
...
sub necessary {
    ...
    return int get_specifications($log, $dbh, $pid, $item, 'txtSpecialPacking');
}


# Packing charges are per second per item, the handling rate is stored in
# dollars per hour.
sub calc {
    ...
    my $seconds 
        = get_specifications($log, $dbh, $pid, $item, 'txtSpecialPacking');
    
    return 'error' unless $seconds;

    my $make_ready = get_price($log, $dbh, $variable, 'PackingMakeReady');
    my $run_rate   = get_price($log, $dbh, $variable, 'Packing');
