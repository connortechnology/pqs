...
sub necessary {
  ...
  foreach my $sig ( eprint::project::get_signature_indices( $log, $dbh, $pid ) ) {
    ...
    my $plate_type = eprint::equipment::get_specification( $log, $dbh, 'Plate Type', '', $press ) if $press;
    if ( $plate_type and ($plate_type eq 'Conventional' || $plate_type eq 'CTP' || $plate_type eq 'DI')) {
...
sub calc { 
	...
	foreach my $sig ( eprint::project::get_signature_indices( $log, $dbh, $pid ) ) {
		my ($qty, $material_id, $signature_quantity) = get_specifications( $log, $dbh, undef, $sig, qw( txtPlateQuantity hdnPlateMaterialID txtSignatureQuantity ));
