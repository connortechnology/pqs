...
sub project_cost {
  ...
  foreach my $j ( @jobs ) {
    ...
    if ( not @competitors ) {
      ...
      @cutters = grep { $_->{supplier} ne $j->supplier and $_->{supplier} ne 'House'} @cutters;
...
sub fits {
  ...
  return 0 unless $job->height < $blade_length or $job->width < $blade_length;
...
sub project_jobs {
  ...
  return (wantarray ? () : undef) if grep { $project->{type} eq $_ } qw( Envelopes InkjetOutputs Product ScreenItem );

  # SIGNATURES
  for my $sig (@{ $project->{signatures} }) {
    my $stock = $sig->{stock}; # Signature stock info.

    # If a project has the same width and height as the press sheet it shouldn't even be here.
    next if  $project->{width}  == $stock->{press}{width} and $project->{height} == $stock->{press}{height};
...
sub cuts {
  ...
  my ($x, $y) = $imposition->{rotate_sheet} ? qw(height width) : qw(width height);
  ...
  $count = sub {
    ...
    for my $i (0 .. $#{ $edges }) {
      ...
      if ($child->is_empty || $sibling->is_empty) {
        $cuts += 1;
      } elsif ($child->bleed->[ (FACING_SIDES)[$dir]->[0] ] || $sibling->bleed->[ (FACING_SIDES)[$dir]->[1] ]) {
        # If either sibling has a bleed between their facing edges we need two cuts to separate them.
        $cuts += 2;
      } else { 
