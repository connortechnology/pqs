...
sub validate_fold_calliper {
  ...
  convert_signatures( $log, $dbh, $pid, $sid, \%specs, \%conversions ) if ( keys %conversions );
...
sub calc {
  ...
  die "Folding called with PID ($pid) or SID ($sid)" unless $pid && $sid;

  my $valid_price = 0;

  my @qtys = (undef, get_quantities($log, $dbh, $pid));

  eprint::Service::Folding::init($log, $dbh);
  ...
  foreach my $i ( 1 .. 3 ) {
    ...
    $openprint::log->debug("HAVE VALID PRICE: $total  ************** $valid_price ");
  }
...
sub get_fold_prices {
  ...
  my @equipment = eprint::service::valid_equipment($log, $dbh, 'Folding', $pid);
  ...
  foreach my $eid (@equipment) {
    ...
    foreach my $name ( keys %$specs ) {
      ...
      my $run_speed = eprint::equipment::get_specification($log, $dbh, "$type Run Speed", $flat_width, $eid);
      ...
      if ($runUnits eq 'Per 1000' || $runUnits eq 'Per 1000 Sheets') {
        if ( !eprint::equipment::equipment_fits( $log, $dbh, $eid, $flat_width, $flat_height)) {
          next EQUIPMENT;
        }
        ...
        my $gap = eprint::equipment::get_specification( $log, $dbh, 'Default Gap', undef, $eid) || DEFAULT_GAP;
        my $run_speed = eprint::equipment::get_specification( $log, $dbh, "$type Run Speed", $flat_width, $eid);
      ...
      RUN:
      foreach my $i ( 1 .. 3 ) {
        next RUN if $specs->{"chkOverrideEquipment$i"} eq 'Y' && $specs->{"ddmEquipment$i"} ne $eid;
        ...
        $qty *= $specs->{txtPressSheetComboItems} if $specs->{txtPressSheetComboItems};

        next RUN unless $qty && $qty > 0;

        $service->{run_price}{$i} += $price * $qty * ($signature_fold_quantity || 1);
      }
...
sub init {
  ...
  $supplier{$_->[0]} = $_->[1] for @{ $openprint::dbh->selectall_arrayref(qq{
  SELECT strid, strsupplier FROM tbl_equipment
  }, undef)};
  $etype{$_->[0]} = $_->[1] for @{ $openprint::dbh->selectall_arrayref(qq{
...
sub best_price {
  ...
  foreach my $price (@{$prices}) {
    ...
    $price->{total}{$qty_index} = $make_ready + $price->{run_price}{$qty_index};

    my $totalPrice        = $price->{total}{$qty_index};
    my $current_bestPrice = $have_price ?  $bestPrice->{total}{$qty_index} : 0;   # actually extract the best price
    ...
    if (    $etype{$price->{equipment}} eq 'digital' or $etype{$price->{equipment}} eq 'web') {
...
sub get_folding_imposition {
  ...
  if (
    ( $orientation eq 'Horizontal' && $flat_height eq $final_height && $flat_width ne $final_width)
    ||
    ( $orientation eq 'Vertical' && $flat_width eq $final_width && $flat_height ne $final_height )
  ) {
