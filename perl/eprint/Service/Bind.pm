...
sub calc {
  ...
  @project{ qw(width height) } = get_specifications($log, $dbh, $pid, $print, qw(final_width final_height));
  ...
  my $inserts = get_specifications($log, $dbh, $pid, $print, 'txtInsertQuantity');
...
#print STDERR "EQUIPEMENT: ", Dumper(@eids);
  foreach my $n ( 1 .. 3 ) {
    $$specs{"hdnBreakdown$n"} = '';
  }

  my %supplier;
  $supplier{$_->[0]} = $_->[1] for @{ $dbh->selectall_arrayref(qq{ SELECT lngindex, strsupplier FROM tbl_equipment }, undef)};
  ...
  foreach my $eid (@eids) {
    ...
    if ( $type eq 'digital' ) {
      my $max_calliper = eprint::equipment::get_specification( $log, $dbh, 'Maximum Stitching Calliper', undef, $eid);
    ...
    } elsif (eprint::equipment::equipment_fits($log, $dbh, $eid, @project{qw(width height calliper)}, 0) ) {
      ...
      if ($$specs{'chkOverrideEquipment1'} or @eids==1) {
        my $equipment = new openprint::Equipment($eid);
        $$specs{hdnBreakdown1} .= "Doesn't fit: " .$equipment->fits(@project{qw(width height calliper)});
      }
    }
  ...
  foreach my $n ( 1 .. 3 ) {
    ...
    $specs->{error} .= 'There is no equipment to handle your job.' if !@eids;
    ...
    foreach my $eid (@eids) {
      ...
      # Perfect binders always have a seperate cover pocket, sticther # need to be checked.
      my $cover_pocket = ($service_type ne 'PerfectBinding') ? eprint::equipment::get_specification($log, $dbh, 'Cover Pocket', undef, $eid) eq 'Y' : 1;
...
# Start binder size selection.
        ...
        $specs->{'hdnRunSpeed'} = eprint::equipment::get_specification( $log, $dbh, 'Run Speed', $pockets, $eid);
...
#        if ($best{price} && $best{price} > 0) {
    if ($best{equipment} ) {
      ...
      $status = 'calculated' if $status ne 'uncalculated';
    } else { 
...
#unless $status eq 'error'; 
    ...
    @$specs{"txtPrice$n", "txtUnitPrice$n"} = format_pricing($best{price}, $qty);
