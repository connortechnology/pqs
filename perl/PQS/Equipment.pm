...
sub get_equipment {
    ...
    croak "Equipment ID not provided." 
        unless defined $id && looks_like_number($id);
    ...
    die "'services' is a reserved key (used erroneously by equipment $id)"
        if exists $equip->{services};

    $equip->{services} 
        = [ map { dualvar $_->[0], $_->[1] } 
               @{ $dbh->selectall_arrayref($services, undef, $id) } ];

	$equip->{invalid_substrates} = $dbh->selectcol_arrayref(q{
		SELECT paper FROM equipment_paper_exclusion 
		WHERE equipment = ?
	},undef,$equip->{id});
