...
%fields = (
  ...
  invoice_comments  => 'strinvoicecomments',
	design					=>	'strdesign',
	created_on			=>	'dtmcreationdate',
	updated_on			=>	'dtmlastmodified',
  shipped_on      =>  'dtmshipdate',
...
%find_fields = (
	...
	estimated_press_name	=>	q`(SELECT strValue FROM tbl_Service_Specifications WHERE lngProjectIndex=`.$table.'.'.$fields{id}.q` AND strName IN ('ddmPress1','ddmPress2','ddmPress3'))`,
	estimated_press_id	=>	q`(SELECT strValue FROM tbl_Service_Specifications WHERE lngProjectIndex=`.$table.'.'.$fields{id}.q` AND strName IN ('press'))`,
