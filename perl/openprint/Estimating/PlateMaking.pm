package openprint::Estimating::PlateMaking;
use strict;
use warnings;
no warnings qw(uninitialized);

use openprint::service   qw(:common);
...
sub necessary {
	...
	my $type = openprint::project::get_type($log, $dbh, $pid);
	return 0 if $type eq 'NoPrint';

    foreach my $sig ( openprint::project::get_signature_indices( $log, $dbh, $pid ) ) {
		my ( $press ) = openprint::service::get_specifications( $log, $dbh, undef, $sig, 'hdnPress' );
		my $plate_type = openprint::equipment::get_specification( $log, $dbh, 'Plate Type', '', $press );
